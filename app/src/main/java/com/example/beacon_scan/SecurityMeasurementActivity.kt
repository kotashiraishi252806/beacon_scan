...
class SecurityMeasurementActivity : AppCompatActivity() {
    ...
    private val securityGroups = mutableListOf<SecurityGroup>()
    private lateinit var adapter: SecurityGroupAdapter
    ...
    private var selectedSecurityGroups: Set<String> = emptySet()
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_security_measurement)
        ...
        switchSendMode.isChecked = prefs2.getBoolean(KEY_SECURITY_SEND_MODE, true)
        switchSendMode.setOnCheckedChangeListener { _, isChecked ->
            if (!isProgrammaticSendModeChange) {
                getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
                    .edit().putBoolean(KEY_SECURITY_SEND_MODE, isChecked).apply()
            }
            if (!isChecked) updatePendingCount()
        }

        adapter = SecurityGroupAdapter(securityGroups) { count ->
        ...
        btnEndSession.setOnClickListener {
            ...
            selectedSecurityGroups = emptySet()
            hideScanResultsView()
        ...
        etUrl.setText(prefs.getString(KEY_SECURITY_SERVER_URL, DEFAULT_SECURITY_SERVER_URL))

        etUrl.setOnEditorActionListener { _, actionId, _ ->
            if (actionId == android.view.inputmethod.EditorInfo.IME_ACTION_DONE) saveUrl()
            false
        }
        ...
        btnMeasureSelected.setOnClickListener {
            ...
            val groups = adapter.getSelectedGroups()
    ...
    private fun showHelpDialog() {
        val msg = """
【概要】
スキャンしたAPをcapabilities_rawからOpen/WPA-PSK/WPA2-PSK/WPA2-EAP/WPA3-SAEの5規格に分類し、
選択した規格のAPすべてに順番にサプリカント接続試行を行います。

━━━━━━━━━━━━━━━━━━
【使い方】
━━━━━━━━━━━━━━━━━━
1. スキャンボタンでAPを検出
2. 測定したいセキュリティ規格を選択して「セッション開始」
3. 測定完了後、場所を移動して「移動完了・測定開始」
4. 同じ規格グループを自動で再測定
5. 「セッション終了」でラベルを設定して保存
        """.trimIndent()
    ...
    private fun saveUrl() {
        ...
        if (url.isNotEmpty()) {
            getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
                .edit().putString(KEY_SECURITY_SERVER_URL, url).apply()
        }
    }

    private fun getServerUrl(): String {
        val saved = getSharedPreferences(PREFS_NAME, Context.MODE_PRIVATE)
            .getString(KEY_SECURITY_SERVER_URL, DEFAULT_SECURITY_SERVER_URL) ?: DEFAULT_SECURITY_SERVER_URL
        return saved.ifEmpty { DEFAULT_SECURITY_SERVER_URL }
    }
    ...
    private fun showAutoLabelDialog(sessionId: String, startTime: Date) {
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("そのまま保存") { _, _ ->
                lifecycleScope.launch {
                    withContext(Dispatchers.IO) {
                        ScanStore.updateLabelBySessionId(this@SecurityMeasurementActivity, sessionId, autoLabel, PENDING_SECURITY_FILE)
                    }
                    updatePendingCount()
                }
            }
            .setNegativeButton("変更して保存") { _, _ ->
                ...
                lifecycleScope.launch {
                    withContext(Dispatchers.IO) {
                        ScanStore.updateLabelBySessionId(this@SecurityMeasurementActivity, sessionId, customLabel, PENDING_SECURITY_FILE)
                    }
                    updatePendingCount()
                }
            }
    ...
    private fun showDiscardSelectionDialog() {
        val byLabel = ScanStore.countByLabel(this, PENDING_SECURITY_FILE)
        ...
        AlertDialog.Builder(this)
            ...
            .setPositiveButton("破棄する") { _, _ ->
                ...
                ScanStore.removeByLabels(this, selectedLabels, PENDING_SECURITY_FILE)
                updatePendingCount()
            }
    ...
    private fun hideScanResultsView() {
        tvStatusCount.visibility = View.GONE
    ...
    private fun updatePendingCount() {
        val count = ScanStore.count(this, PENDING_SECURITY_FILE)
        if (count == 0) {
    ...
    private fun startMeasurementCycle(targetAps: List<AccessPoint>, location: Location?, scanId: String, allScannedAps: List<AccessPoint> = emptyList()) {
        ...
        val groupCount = selectedSecurityGroups.size
        val apCount = targetAps.size

        securityGroups.clear()
        securityGroups.addAll(groupBySecurityGroup(targetAps))
        ...
        tvStatusCount.text = "セッション測定中 (${groupCount}規格, 計${apCount}台)..."
        tvStatusCount.visibility = View.VISIBLE
        ...
        lifecycleScope.launch {
            val measuredList = measurer.measureAll(
                ...
                onProgress = { progress, total, name ->
                    ...
                    tvStatusCount.text = "セッション測定中 $progress/$total (残り${remaining}件): $name"
                },
                onApStart = { ap ->
            ...
            withContext(Dispatchers.IO) {
                saveToPending(measuredList, location, scanId, sid, selectedSecurityGroups.toList(), allScannedAps)
            }
            updatePendingCount()
            ...
            tvStatusCount.text = "測定完了 (${groupCount}規格, 計${apCount}台)\n次の測定場所に移動してください"
    ...
    private fun onScanResultsReady() {
        ...
        Log.d("BeaconScan", "WiFiスキャン完了: ${System.currentTimeMillis() - scanStartMs}ms")
        ...
        securityGroups.clear()
        securityGroups.addAll(groupBySecurityGroup(grouped))

        val location = latestLocation
        val scanId = UUID.randomUUID().toString()
        val label = autoScanSessionId ?: ""
        val groupCount = securityGroups.size
        ...
        if (apList.isEmpty()) {
            ...
            isProcessingResults = false
            if (sessionId != null) {
                tvStatusCount.text = "APが検出されませんでした\n次の測定場所に移動してください"
                tvStatusCount.visibility = View.VISIBLE
                layoutMoveButtons.visibility = View.VISIBLE
                btnScan.isEnabled = false
                tvEmpty.visibility = View.GONE
        ...
        if (switchAutoScan.isChecked) {
            adapter.isSelectionMode = false
            adapter.notifyDataSetChanged()
            tvStatusCount.text = "今回検出: ${groupCount}規格 (${apCount}台)\nSupplicant測定中..."
            tvStatusCount.visibility = View.VISIBLE
            btnToggleList.visibility = View.VISIBLE
            btnStopMeasurement.visibility = View.VISIBLE
            btnStopMeasurement.isEnabled = true
            ...
            lifecycleScope.launch {
                ...
                val totalRecords = withContext(Dispatchers.IO) { ScanStore.totalRecords(this@SecurityMeasurementActivity, PENDING_SECURITY_FILE) }
            ...
            // セッション2回目以降：前回選択セキュリティ規格で自動測定
            if (sessionId != null && selectedSecurityGroups.isNotEmpty()) {
                val targetAps = apList.filter { getSecurityGroup(it.capabilitiesRaw) in selectedSecurityGroups }
                isScanInProgress = false
                isProcessingResults = false
                if (targetAps.isNotEmpty()) {
                    ...
                    lifecycleScope.launch {
                        withContext(Dispatchers.IO) {
                            saveToPending(emptyList(), location, scanId, sid, selectedSecurityGroups.toList())
                        }
                        updatePendingCount()
                        if (switchSendMode.isChecked) trySendAllPending()
                    }
                    tvStatusCount.text = "選択済み規格のAPが検出されませんでした\n次の測定場所に移動してください"
                    tvStatusCount.visibility = View.VISIBLE
                    layoutMoveButtons.visibility = View.VISIBLE
                    tvEmpty.visibility = View.GONE
                    AlertDialog.Builder(this)
                        .setTitle("選択済み規格のAPが検出されませんでした")
                        .setMessage("対象規格: ${selectedSecurityGroups.joinToString(", ")}")
                        .setPositiveButton("OK", null)
                        .show()
                }
                return
    ...
    private fun saveToPending(
        ...
        scanId: String,
        label: String,
        targetedSecurityGroups: List<String>? = emptyList(),
        scanResults: List<AccessPoint> = emptyList()
    ) {
        ...
        val entry = JSONObject().apply {
            ...
            put("label", label)
            if (targetedSecurityGroups != null) put("targeted_security_groups", JSONArray(targetedSecurityGroups))
            else put("targeted_security_groups", JSONObject.NULL)
            if (location != null) {
        ...
        ScanStore.append(this, entry, PENDING_SECURITY_FILE)
    }

    private suspend fun trySendAllPending() {
        val pending = withContext(Dispatchers.IO) { ScanStore.load(this@SecurityMeasurementActivity, PENDING_SECURITY_FILE) }
        ...
        for (i in 0 until total) {
            ...
            if (success) {
                withContext(Dispatchers.IO) { ScanStore.removeFirst(this@SecurityMeasurementActivity, PENDING_SECURITY_FILE) }
            } else {
